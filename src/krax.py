...
class FILTER(SFC):
  en = POU.input(False,hidden=False)      # включить логику
  q  = POU.output(False,hidden=True)      # включить вибратор
  pt = POU.var(int(5),persistent=True)    # время вкллючения вибратора фильтра
  wt = POU.var(int(10),persistent=True) # пауза между включениями
  
  def __init__(self,en:bool=False,q:bool=None,id: str=None,parent:POU=None):
  ...
  def main(self):
    self.q = False
    yield from self.until(lambda: self.en)
    self.q = True
    sec = 0
    while sec<self.pt and self.en:
      yield from self.pause(1000, step='powered')
      sec+=1
      
    self.q = False
    sec = 0
    while sec<self.wt and self.en:
      yield from self.pause(1000,step='pause')
      sec+=1
      
    yield #если все времена =0 чтобы избежать бесконечный цикл

if platform=='vscode':
  PLC = namedtuple('PLC', ('FILLERS_M_1', 'CEMENT_M_1', 'WATER_M_1', 'MIXER_I_1', 'MIXER_ON_1', 'APUMP_ON_1', 'CONVEYOR_ON_1', 'ELEVATOR_UP_1', 'ELEVATOR_DOWN_1', 'VIBRATOR_ON_1', 'VIBRATOR_ON_2', 'VIBRATOR_ON_3', 'S_VIBRATOR_ON_1', 'AUGER_ON_1', 'FILTER_ON_1', 'FILLER_OPEN_1', 'FILLER_OPEN_2', 'FILLER_OPEN_3', 'DCEMENT_OPEN_1', 'WATER_OPEN_1', 'DWATER_OPEN_1', 'MIXER_OPEN_1',
                  'EMERGENCY_ON_1', 'MIXER_ISON_1', 'AUGER_ISON_1', 'CONVEYOR_ISON_1', 'ELEVATOR_RISE_1', 'ELEVATOR_LOWER_1', 'FILLER_CLOSED_1', 'FILLER_CLOSED_2', 'FILLER_CLOSED_3', 'APUMP_ISON_1', 'DCEMENT_CLOSED_1', 'DWATER_CLOSED_1', 'WATER_CLOSED_1', 'MIXER_CLOSED_1', 'MIXER_OPENED_1', 'ELEVATOR_BELOW_1', 'ELEVATOR_MIDDLE_1', 'ELEVATOR_ABOVE_1', 'LLEVEL_1', 'HLEVEL_1', 'EMERGENCY_ISON_1'))
  plc=PLC()
...
water_1 = Container(m=lambda: water_m_1.m,closed=~plc.WATER_OPEN_1,out=plc.WATER_OPEN_1,lock=Lock(key=lambda: plc.APUMP_ON_1 or not plc.DWATER_CLOSED_1),max_sp=300)
addition_1 = Container(m=lambda: water_m_1.m,closed=~plc.APUMP_ON_1,out = plc.APUMP_ON_1,lock=Lock(key=lambda: plc.WATER_OPEN_1 or not plc.DWATER_CLOSED_1),max_sp=50)
dwater_1 = Dosator(m=lambda: water_m_1.m, out=plc.DWATER_OPEN_1, closed=plc.DWATER_CLOSED_1,containers=(water_1,addition_1),lock=Lock(key=lambda: plc.WATER_OPEN_1 or plc.APUMP_ON_1 or not plc.MIXER_ISON_1))
...
filter_1 = FILTER(q=plc.FILTER_ON_1)

instances = (factory_1,motor_1,gate_1,mixer_1,cement_m_1,water_m_1,fillers_m_1,silage_1,water_1,addition_1,filler_1,filler_2,filler_3,dcement_1,dwater_1,conveyor_1,elevator_1,ready_1,loaded_1,manager_1,vibrator_1,vibrator_2,vibrator_3,retarder_1,emergency_move_1,filter_1 )
...
if platform=='linux':
  ...
  iwater_m_1 = iWEIGHT(speed=100, loading=lambda: plc.WATER_OPEN_1 or plc.APUMP_ON_1,unloading=plc.DWATER_OPEN_1,q=plc.WATER_M_1)
  ...
  instances += (imotor_1,igate_1,idcement_1,idwater_1,iconveyor_1,iauger_1,iapump_1,ifiller_1,ifiller_2,ifiller_3,ielevator_1,icement_m_1,iwater_m_1,ifillers_m_1,ielevator_up_1,ielevator_down_1,iwater_1)
